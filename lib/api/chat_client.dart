...
import 'package:hackathon/constants/appwrite_constants.dart';
...
class ChatServicesNotifier extends StateNotifier<List<ChatBubble>> {
  ...
  ChatBubble _parseChat(Chat chat) {
    return ChatBubble(
      ...
      alignment:
          user!.uid == chat.senderid ? Alignment.topRight : Alignment.topLeft,
      shadowColor: Colors.transparent,
      backGroundColor:
          user!.uid != chat.senderid ? Colors.grey : Colors.lightBlue,
      clipper: ChatBubbleClipper1(
        type: user!.uid == chat.senderid
  ...
  void _getOldMessages(UserModel? user) async {
    try {
      final models.DocumentList temp = await database.listDocuments(
          databaseId: AppwriteConstants.databaseId, collectionId: collectionId);
